...
EXEC_PATH=${SERVER_HOME}/bin/kaa-sandbox-start
JAVA_OPTIONS=
...
start() {
  ...
  /bin/su -s /bin/bash -c "/bin/bash -c 'echo \$\$ >${SERVER_PID_FILE} && exec ${EXEC_PATH} >>${SERVER_LOG_DIR}/${NAME}-server${SERVER_LOG_SUFIX}.init.log 2>&1' &" $SERVER_USER
  RETVAL=$?
...
stop() {
  ...
  if [ -n $SERVER_PID ]; then
    /bin/su -s /bin/bash -c "kill -TERM ${SERVER_PID} >>${SERVER_LOG_DIR}/${NAME}-server${SERVER_LOG_SUFIX}.shutdown.log 2>&1" $SERVER_USER
